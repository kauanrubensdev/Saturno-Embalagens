  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#fcfbf8' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-sm mb-6" style={{ color: '#999999' }}>
          <Link to="/" className="no-underline transition-colors hover:opacity-80" style={{ color: '#999999' }}>Início</Link>
          <span>/</span>
          <Link to="/catalog" className="no-underline transition-colors hover:opacity-80" style={{ color: '#999999' }}>Catálogo</Link>
          <span>/</span>
          <span style={{ color: '#1a1a1a' }}>{categoryName || categorySlug}</span>
        </nav>
        ...
        <div className="mb-8">
          <div className="relative max-w-md">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: '#999999' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <p className="text-base font-medium mb-2" style={{ color: '#991b1b' }}>{error}</p>
            <button onClick={() => window.location.reload()} className="text-sm font-medium cursor-pointer" style={{ color: '#FF6B00' }}>
              Tentar novamente
            </button>
          </div>
          ...
          <div className="text-center py-16">
            ...
            {search ? (
              <button onClick={() => setSearch('')} className="text-sm font-medium cursor-pointer" style={{ color: '#FF6B00' }}>
                Limpar busca
              </button>
            ) : (
              <Link to="/catalog" className="text-sm font-medium no-underline" style={{ color: '#FF6B00' }}>
